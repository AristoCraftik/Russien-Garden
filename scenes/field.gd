...
func plant_seed(cell_pos: Vector2i, plant_data: PlantData, growth_stage: int = 0, watered_state: bool = false) -> bool:
	if plant_data == null:
		return false
...
func harvest_plant_at(cell_pos: Vector2i) -> HarvestOutcome:
	...
	if not inventory.has_method("try_add_items") or not inventory.try_add_items(yield_res as ItemData, qty, drop_from):
		...
		push_warning("Инвентарь полон — освободите слот, чтобы собрать урожай.")
