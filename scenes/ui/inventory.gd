...
func fly_item_to_slot(slot_index: int, texture: Texture2D) -> void:
	...
	fly_item.size = slot.size * 0.5
	fly_item.mouse_filter = Control.MOUSE_FILTER_IGNORE
	
	# stretch mode для красивой текстуры
	fly_item.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	
	get_tree().root.add_child(fly_item)
	fly_item.global_position = get_viewport().get_visible_rect().size / 2 - fly_item.size / 2
	
	var target_pos = slot.global_position + slot.size / 2 - fly_item.size / 2
	
	var tween = create_tween()
	tween.set_ease(Tween.EASE_OUT)
	tween.set_trans(Tween.TRANS_BACK)
	tween.tween_property(fly_item, "global_position", target_pos, 0.5)
	...
	tween.tween_callback(func():
		...
		# Привязываем скрипт перетаскивания
		...
		item.offset_left = -item.size.x / 2
		item.offset_top = -item.size.y / 2
		item.offset_right = item.size.x / 2
		item.offset_bottom = item.size.y / 2
	)
